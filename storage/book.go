...
func GetByIdBook(db *sql.DB, req models.BookPrimeryKey) (models.BookCategory, error) {

	var (
		book models.BookCategory
	)
	...
	query1 := `select
                      ...
                      c.name
               from category_book as cb
               join category as c on c.id = cb.category_id
               where cb.bookId = $1`

	rows, err := db.Query(query, req.Id)
	if err != nil {
		return models.BookCategory{}, err
	}
	...
	for rows.Next() {
		...
		for rows.Next() {
			var catalog models.Categories
			err = rows.Scan(
				&catalog.Id,
				&catalog.Name,
			)
			book.Categorys = append(book.Categorys, catalog)
		}
	}
	if err != nil {
		return models.BookCategory{}, err
